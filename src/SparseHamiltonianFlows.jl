module SparseHamiltonianFlows
    ...
    function log_joint_density_for_elbo(a::Args, z::Vector{Float64})
        ...
        # for i in 1:a.n_subsample_elbo
        #     sub_xs_i = ones(dx)
        #     ignore() do
        #         sub_xs_i = sub_xs_elbo[i,:]
        #     end
        #     πs = πs + a.logp_lik(sub_xs_i, z) * (a.N / a.n_subsample_elbo)
        # end
        return πs
    end
    ...
    function adaptive_refresh(a::Args, ϵ_unc, w_unc, refresh_sample_size, kl_threshold)
        refresh_iters = []
        refresh = false
        terminate = false
        r_states = []
        ...
        ∇U = (zz, ww) -> a.∇potential_by_hand(a.sub_xs, zz, ww)
        zs_ref = a.sample_q0(refresh_sample_size)
        ps_ref = randn(refresh_sample_size, a.d)

        while !terminate
            counter = 0
            refresh = false
            while !refresh
                zs_ref_prev = copy(zs_ref)
                ps_ref_prev = copy(ps_ref)
                kl_prev = kl_gaussian(ps_ref_prev)

                for i in 1:refresh_sample_size
                    oneleapfrog!(@view(zs_ref[i,:]), @view(ps_ref[i,:]), ϵ_unc, ∇U, w_unc)
                end

                kl_curr = kl_gaussian(ps_ref)
                println(kl_curr)
                if kl_prev >= kl_curr
                    refresh = true
                    zs_ref = zs_ref_prev
                    ps_ref = ps_ref_prev
                    println("peaked")
                    if length(refresh_iters) == 0
                        push!(refresh_iters, counter)
                    else
                        push!(refresh_iters, refresh_iters[end] + counter)
                    end
                    if kl_prev <= kl_threshold
                        terminate = true
                    end
                else
                    counter += 1
            ...
            push!(r_states, IdDict())
            μz, μp, Σpz, Σz, Σp = compute_mv(zs_ref, ps_ref)
            get!(r_states[end], "key")do
                (μz, μp, Σpz, Σz, Σp, [counter])
            end
            batch_refresh!(a, μz, μp, Σpz, Σz, Σp, zs_ref, ps_ref)
        end

        return refresh_iters, r_states
    end

    function sample_from_adaptive_refresh(a::Args, num_sample::Int64, ϵ_unc_final::Vector{Float64}, w_unc_final::Vector{Float64}, r_states::Vector{Any}, refresh_iters::Vector{Any})
        zs = a.sample_q0(num_sample)
        ps = randn(num_sample, a.d)
        zs, ps = trained_flow_adaptive(a, zs, ps, ϵ_unc_final, w_unc_final, r_states, refresh_iters)
        return zs, ps
    end

    function trained_flow_adaptive(a::Args, zs::Matrix{Float64}, ps::Matrix{Float64}, ϵ_unc_final::Vector{Float64}, w_unc_final::Vector{Float64}, r_states::Vector{Any}, refresh_iters::Vector{Any})
        ∇U = (zz, ww) -> a.∇potential_by_hand(a.sub_xs, zz, ww)
        num_sample = size(zs,1)
        for n in 1:refresh_iters[end]
            for i in 1:num_sample
                oneleapfrog!(@view(zs[i,:]), @view(ps[i,:]), ϵ_unc_final, ∇U, w_unc_final)
            end

            if n in refresh_iters
                r = argmax(n .== refresh_iters)
                r_state = r_states[r]
                μz, μp, Σpz, Σz, Σp, _ = get!(r_state, "key", 0)
                Σ_inv_sqrt, Σ_mean, _ = compute_transformation(Σpz, Σz, Σp, a.cond)
                if a.cond
                    for i in 1:num_sample
                        @view(ps[i,:]) .= Σ_inv_sqrt * (@view(ps[i,:]) .- (μp .+ Σ_mean * (@view(zs[i,:]) .- μz)))
                    end
    ...
    function appended_flow(a::Args, ϵ_unc, w_unc, refresh_sample_size, kl_threshold, r_states::Vector{Any}, cond)
        zs_ref, ps_ref = sampler(a, refresh_sample_size, ϵ_unc, w_unc, r_states)
        a.cond = cond
        terminate = false
        r_states = []
    ...
    function sample_from_appended_flow(a::Args, zs::Matrix{Float64}, ps::Matrix{Float64}, ϵ_unc::Vector{Float64}, w_unc::Vector{Float64}, r_states::Vector{Any})
        ∇U = (zz, ww) -> a.∇potential_by_hand(a.sub_xs, zz, ww)
        num_sample = size(zs,1)
        for n in 1:(a.K * length(r_states))
            for i in 1:num_sample
                oneleapfrog!(@view(zs[i,:]), @view(ps[i,:]), ϵ_unc, ∇U, w_unc)
            end
